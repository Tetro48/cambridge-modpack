...
local ChallengerGrid = Object:extend()
...
function ChallengerGrid:getCell(x, y)
...
function ChallengerGrid:isOccupied(x, y)
...
function ChallengerGrid:canPlacePiece(piece)
	if piece.big then
...
function ChallengerGrid:canPlaceBigPiece(piece)
	local offsets = piece:getBlockOffsets()
	for index, offset in pairs(offsets) do
		local x = piece.position.x + offset.x
...
function ChallengerGrid:canPlacePieceInVisibleGrid(piece)
	if piece.big then
		return self:canPlaceBigPiece(piece)
	...
	for index, offset in pairs(offsets) do
		...
		if y < 20 or self:isOccupied(x, y) ~= empty then
			return false
		end
	end
	return true
...
function ChallengerGrid:clearClearedRows()
	for row = 1, self.height do
		if self.cleared_lines[row] then
			self.cleared_lines[row] = false
...
function ChallengerGrid:copyBottomRow()
...
function ChallengerGrid:garbageRise(row_vals)
	for row = 1, self.height - 1 do
		self.grid[row] = self.grid[row + 1]
		self.grid_age[row] = self.grid_age[row + 1]
	end
...
function ChallengerGrid:clearSpecificRow(row)
	for col = 1, self.width do
		self.grid[row][col] = empty
	end
end

function ChallengerGrid:applyPiece(piece)
	if piece.big then
...
function ChallengerGrid:checkForBravo(cleared_row_count)
...
function ChallengerGrid:checkStackHeight()
...
function ChallengerGrid:checkSecretGrade()
	local sgrade = 0
	for i = 23, 5, -1 do
...
function ChallengerGrid:hasGemBlocks()
	for y = 1, self.height do
		for x = 1, self.width do
...
function ChallengerGrid:mirror()
	local new_grid = {}
	for y = 1, self.height do
		new_grid[y] = {}
		for x = 1, self.width do
...
function ChallengerGrid:applyMap(map)
...
function ChallengerGrid:markSquares()
...
-- square scan
function ChallengerGrid:scanForSquares()
	local table = {}
	for row = 1, self.height do
		local silver = 0
		local gold = 0
...
function ChallengerGrid:update()
	for y = 1, self.height do
		for x = 1, self.width do
		...
		for x = 1, self.width do
			if blocks[self.grid[y][x].skin] and
				...
				if self.grid_age[y][x] < 2 then
					...
					love.graphics.draw(blocks[self.grid[y][x].skin]["F"], 48 + x * 16, y * 16 - 256)
				else
					...
					love.graphics.draw(blocks[self.grid[y][x].skin][self.grid[y][x].colour], 48 + x * 16, y * 16 - 256)
				end
				if self.grid[y][x].skin ~= "bone" and self.grid[y][x].colour ~= "X" then
					...
					if y > 1 and self.grid[y - 1][x] == empty or self.grid[y - 1][x].colour == "X" then
						love.graphics.line(48.0 + x * 16, -0.5 + y * 16 - 256, 64.0 + x * 16, -0.5 + y * 16 - 256)
					end
					if y < self.height and self.grid[y + 1][x] == empty or
						(y + 1 <= self.height and self.grid[y + 1][x].colour == "X") then
						love.graphics.line(48.0 + x * 16, 16.5 + y * 16 - 256, 64.0 + x * 16, 16.5 + y * 16 - 256)
					end
					if x > 1 and self.grid[y][x - 1] == empty then
						love.graphics.line(47.5 + x * 16, -0.0 + y * 16 - 256, 47.5 + x * 16, 16.0 + y * 16 - 256)
					end
					if x < self.width and self.grid[y][x + 1] == empty then
						love.graphics.line(64.5 + x * 16, -0.0 + y * 16 - 256, 64.5 + x * 16, 16.0 + y * 16 - 256)
					end
				end
			end
		end
...
function ChallengerGrid:drawOutline()
	for y = 1, self.height do
		for x = 1, self.width do
...
function ChallengerGrid:drawInvisible(opacity_function, garbage_opacity_function, lock_flash, brightness)
...
function ChallengerGrid:drawCustom(colour_function, gamestate)
	--[[
